...
- **Network senders consuming protected data** — `curl --data @…`, `-F …@…`, `wget --post-file`, `nc` stdin redirection, `scp`; plus list/config carriers (`curl -K` config with `data = @…`, `tar -T`/`--files-from`, `rsync --files-from`, `zip -@` member lists), whose operand bodies are scanned for protected references.
...
- **Git history/object access** — `git show`/`cat-file`/`archive`/`log -p` naming protected paths, behind global flags (`git -C repo show …`) included, plus bare patch-displaying history (`git log -p`, `git show HEAD`) and history packaging (`git bundle create`, `git format-patch`) asking for approval.
...
- **Guard self-protection** — writes to the plugin's override file and heartbeat are denied, writes to plugin/policy sources require approval, and reads stay free so the guard is auditable; `doctor` fails (not warns) on a non-active or dead-pid heartbeat. On beta-19086, write-tool asks (including `GG-SLF-004`) enforce as blocks — approve the equivalent shell form (`vim policy/policy.jsonc`) instead. Trusted locations whose scripts legitimately mention protected names (e.g. this repo's test corpus) can be exempted from the content ask via `contentWriteAllowlist` in the operator-only override file (`security-guard.config.json`) — see the config example.
...
- **File identity survives renaming and directory copies** — `cp .env keyfile && cat keyfile` and `cp -r ~/.ssh /tmp/s && cat /tmp/s/config` block, same-command and cross-call (session store, read/grep tools, permission channel); same-command `rm` clears so reuse stays possible. Directory provenance survives the idiomatic spellings (`cp -r ~/.ssh/ /tmp/t`, `cp -r ~/.ssh /tmp/s2/`, `cp -r ~/.ssh/* /tmp/t`, `mv ~/.ssh/ /tmp/m`) and the two-segment stores (`.config/gcloud`, `.config/secrets`), and it reaches MCP tools (a trusted filesystem-style MCP read of a tracked copy blocks like the native read tool).
