...
- **Protected-file reads** via the native read tool — hard-denied for high-confidence secrets (`.env*`, key material, cloud/SSH/kube/GPG stores, Terraform state, package-manager auth), with sanitized exceptions (`.env.example` et al.) explicitly allowed.
- **Shell-based secret reads** — `cat`/`head`/`tail`/`less`/`grep`/`awk`/`sed` and friends aimed at protected paths are blocked by the execution-time guard, including env-var (`$HOME/.env`) and alternate-separator constructions, temp-copy provenance (`cp .env /tmp/x && curl -d @/tmp/x`), and benign-named symlinks resolving onto protected files.
...
- **Transformations & packing** — `base64`/`xxd`/`od`/`openssl` input-flag misuse and archive creation (`tar`/`zip`/`jar`/`7z`) containing protected members.
...
- **Git history/object access** — `git show`/`cat-file`/`archive`/`log -p` naming protected paths.
- **MCP tools** — per-tool permission rules natively; trust-tier defaults, semantic classes (read-only / external-write / destructive / credential-related / unknown), argument-level protected-path and secret-value rules via the guard.
