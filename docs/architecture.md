...
- a pure decision engine does all analysis (path classification with case folding for APFS/NTFS and shell-word normalization for quoting/backslash artifacts, command segmentation/tokenization, reader/interpreter/transformer/sender/archive/git verb classes, **re-entry wrapper recursion** (`bash -c`, `eval`, `env`, `sudo`, `watch`, `xargs`), env-dump and secret-variable detection (bare `export`/`declare` and `${VAR:-x}` operators included), `$VAR`/`${!VAR}`/`$IFS` indirection, alias-definition bodies, glob-exemplar matching (`GGR-GLOB-001`), broad-root archive/history/search asks (`GGA-DIR-001`, `GGG-HIST-001`, `GGR-SEARCH-001`), procfs rules (`GG-PROC-*`), curated content viewers, same-command temp-copy provenance via tracked `cp` **plus** a bounded session copy store (`createCopyProvenanceStore`, populated post-execution and cleared on `rm`/overwrite), `--kubeconfig` flag pointers, guard self-protection on write-class operations, and protected-reference scanning of script content at write time). It is unit-tested without OpenCode;
- diagnostics are value-free: rule ID, category, matched **basename** only — never command text, variable values, or file contents;
- **fail-open reality**: if the plugin throws during load, OpenCode logs a WARN and continues without it (probed on beta-18219). Countermeasure: the plugin writes a heartbeat JSON (`~/.local/share/security-guard-for-opencode/health.json`) as the very first setup step, and `scripts/doctor.mjs --live` **fails** when it is missing, stale, non-`active`, or records a dead pid, or when logs show load failures.
